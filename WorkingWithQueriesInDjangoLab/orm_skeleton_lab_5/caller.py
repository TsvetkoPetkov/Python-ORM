...
def find_books_by_genre_and_language(book_genre: str, book_language: str):
    searched_book = Book.objects.filter(genre=book_genre, language=book_language)
...
def find_authors_nationalities():
    not_null_nationality_authors = Author.objects.exclude(nationality__isnull=True)

    authors_info_to_return = []

    for author in not_null_nationality_authors:
        authors_info_to_return.append(
            f"{author.first_name} {author.last_name} is {author.nationality}"
        )

    return '\n'.join(authors_info_to_return)
...
def order_books_by_year():
    ...
    books_info_to_return = []

    for book in ordered_books:
        books_info_to_return.append(
            f"{book.publication_year} year: {book.title} by {book.author}"
        )

    return '\n'.join(books_info_to_return)
...
# print(delete_review_by_id(9))
# print(delete_review_by_id(3))
# print(delete_review_by_id(5))
...
def filter_authors_by_birth_year(start_date, end_date):
    authors_born_between = Author.objects.filter(
        birth_date__year__range=(start_date, end_date)
    ).order_by('-birth_date')

    authors_info_to_return = []

    for author in authors_born_between:
        authors_info_to_return.append(
