...
def show_highest_rated_art():
    ...
    return (f"{highest_rated_art.art_name} is the "
            f"highest-rated art with a "
            f"{highest_rated_art.rating} rating!")
...
def show_the_most_expensive_laptop():
    ...
    return (f"{most_expensive_laptop.brand} is the most expensive "
            f"laptop available for {most_expensive_laptop.price}$!")
...
def show_workouts():
    needed_types = Workout.objects.filter(workout_type__in=["Calisthenics", "CrossFit"])

    needed_types_info_to_return = []

    for wt in needed_types:
        needed_types_info_to_return.append(
            f"{wt.name} from {wt.workout_type} type has {wt.difficulty} difficulty!"
        )

    return '\n'.join(needed_types_info_to_return)
